...
class UtteranceHandler:
    ...
    def get_random_number(self, param_dict):
        ...
        rand_num = str(random.randint(num_min, num_max))

        while rand_num == self.key_index:
            rand_num = str(random.randint(num_min, num_max))
        return rand_num
    ...
    def replace_param(self, utt, param_name, param_dict, utterance_mode=False, replace_num=-1):
        """
        -*-逻辑方法-*-
        ...
        if utterance_mode:
            ...
            key = '({param})[{param_name}]'.format(param=param, param_name=param_name)
        param_mark = '[{param_name}]'.format(param_name=param_name)
        utt = utt.replace(param_mark, key, replace_num)
        return utt

    def generate_json(self, utterance_mode=False):
        """
        ...
        with open(self.json_path, 'r', encoding='utf-8') as a_output:
            data = json.load(a_output)
        # 第一层
        for goal in data:
            goal_dict = dict()
            goal_dict["GoalID"] = goal["GoalID"]
